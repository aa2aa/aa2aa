...
$OMARyes = file_get_contents("run");
if($OMARyes == 'yes'){
    $OMARSta = "قيد التشغيل";
}elseif($OMARyes == 'no'){
    $OMARSta = "تم الاطفاء";
}
$OMARold = file_get_contents("oldchannel");
...
- حالة التشيكر => $OMARSta .
- القناة القديمة => @$OMARold .
- التثبيت في => $OMARtype .
- الرقم => $phone .
- الضغطات => $loops . 
- المعرف => @$username . 
...
if(preg_match('/old @/', $text )) { 
  $OMAR = explode('/old @',$text)[1]; 
  file_put_contents("oldchannel",$OMAR); 
  $tg->vtcor('sendMessage',[ 
  'chat_id'=>$chat_id, 
  'text'=>"- تم وضع القناة القديمة => @$OMAR .", 
  ]); 
  } 
  $type = file_get_contents('type');
